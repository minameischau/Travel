...
if ( ! class_exists( 'MoMessages' ) ) {
	...
	final class MoMessages extends BaseMessages {
			...
			define(
				...
				maybe_serialize(
					array(
						...
						// General Messages.
						self::GLOBALLY_INVALID_PHONE_FORMAT => mofr_(
						...
						self::CHANGE_EMAIL_ID_BODY         => mofr_(
						...
						self::INSTALL_PREMIUM_PLUGIN       => mofr_(
							"You have Upgraded to the Firebase Gateway Plan. You will need to 
						...
						// for onprem plugin.
						self::DEFAULT_SMS_TEMPLATE         => mofr_(
						...
						self::DEFAULT_EMAIL_TEMPLATE       => mofr_(
		 ...
		 * @return string
		 */
		public static function show_message( $message_keys, $data = array() ) {
			$display_message = '';
			$message_keys    = explode( ' ', $message_keys );
			$messages        = maybe_unserialize( MOFR_MESSAGES );
			foreach ( $message_keys as $message_key ) {
				if ( MoUtility::is_blank( $message_key ) ) {
					return $display_message;
				}
				$format_message = mofr_( $messages[ $message_key ] );
