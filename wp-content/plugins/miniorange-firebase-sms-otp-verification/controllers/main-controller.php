...
$prem_disabled     = ( MOFLR_LICENSE_TYPE === 'MoFirebasePremiumPlan' && file_exists( MOFLR_DIR . 'helper/class-mofirebasepremiumplan.php' ) ) && ( $registered && $activated ) ? '' : 'disabled';
...
if ( isset( $_GET['page'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.NonceVerification.Recommended -- Reading GET parameter from the URL for checking the tab name, doesn't require nonce verification.
	foreach ( $tab_details->tab_details as $mofr_tabs ) {
		if ( sanitize_text_field( wp_unslash( $_GET['page'] ) ) === $mofr_tabs->menu_slug ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.NonceVerification.Recommended -- Reading GET parameter from the URL for checking the tab name, doesn't require nonce verification.
			include $controller . $mofr_tabs->view;
		}
	}
...
echo '
<div class="w-[368px] contact-us-container duration-300"> 
	<div class="flex gap-mofr-4 relative justify-end">
		<input id="contact-us-toggle" type="checkbox" class="peer sr-only"/>

		<label for="contact-us-toggle" class="flex-1 flex relative items-center h-[68px] bg-gradient-to-r from-[#FF6683] to-[#FF8C8C] px-mofr-4 rounded-smooth shadow-md">
			<span class="mofr-heading text-white leading-normal">Hello there! Need Help?<br>Drop us an Email</span>
		</label>

		<label for="contact-us-toggle">
			<svg width="68" height="68" viewBox="0 0 102 103" fill="none" class="cursor-pointer">
			  ...
			  <defs>
				<linearGradient id="b69bc691e4b17a460c917ded85c3988c" x1="0" y1="0" x2="102" y2="103" gradientUnits="userSpaceOnUse">
				  <stop stop-color="#FF6683"></stop>
				  <stop offset="1" stop-color="#FF8C8C"></stop>
				</linearGradient>
			  </defs>
			</svg>
		</label>
		<div class="overflow-hidden hidden peer-checked:block absolute right-mofr-0 bottom-mofr-0 bg-white shadow-lg rounded-smooth border animate-fade-in-up">
			<div class="mofr-section-header">
				<h5 class="mofr-heading flex-1">Contact us</h5>
				<label class="mofr-icon-button" for="contact-us-toggle">
			...
			<form name="f" method="post" action="" class="flex flex-col gap-mofr-3 p-mofr-6">
				...
				<input type="hidden" name="option" value="mofr_validation_contact_us_query_option"/>
						...
						<input type="email" class="mofr-input" id="query_email" name="query_email" value="' . esc_attr( $email ) . '"
							placeholder="' . esc_attr( mofr_( ' Enter your Email' ) ) . '" required />
				  
					
						<input type="text" class="mofr-input" name="query_phone" id="query_phone" value="' . esc_attr( $phone ) . '"
							placeholder="' . esc_attr( mofr_( ' Enter your phone' ) ) . '"/>
				 
							<textarea id="query" name="query" class="mofr-textarea h-[100px]"
				...
				<input type="submit" name="send_query" id="send_query" value="' . esc_attr( mofr_( ' Submit Query' ) ) . '" 
						class="mofr-button inverted" />
				<a href="mailto:otpsupport@xecurify.com" class="mofr-button secondary">
					<svg width="20" height="20" viewBox="0 0 24 24" fill="none" class="-ml-mofr-4">
